# Claude 全局 Skills 说明

这个目录存放当前用户自定义的 Claude 全局技能。每个技能使用独立目录管理，核心入口文件为 `SKILL.md`。

## 目录用途

- 作用范围：放在 `~/.claude/skills/` 下的技能，可在 Claude 会话中作为全局技能被发现和使用。
- 适用场景：沉淀常用 Git 流程、文档生成、业务核查、设计规范和项目 MCP 接入能力。
- 组织方式：一个技能一个目录；如有脚本、模板、参考文档，跟随技能目录一起维护。

## 当前技能

| 技能目录 | 主要用途 |
| --- | --- |
| `auto-plan-dev` | 仅在用户显式点名时，把已确认需求文档或功能需求生成为可执行的开发计划。 |
| `business-feature-audit` | 对当前改动做业务闭环核查，检查流程、状态、权限、上下游协同和数据一致性。 |
| `development-trace` | 为当前需求生成开发留痕文档，沉淀需求目标、实现过程和结果。 |
| `generate-readme` | 基于项目真实文件和配置生成或更新项目根目录 `README.md`。 |
| `git-commit-msg` | 根据用户确认的 Git 文件范围分析 diff，并生成规范化提交消息。 |
| `git-push` | 从确认文件范围、生成提交信息，到执行 `git add`、`git commit`、`git push` 的完整提交流程。 |
| `process-usage-report` | 分析当前电脑进程使用情况，输出结构化资源占用报告。 |
| `project-design-md-generator` | 为前端项目生成或更新 `DESIGN.md`，沉淀 UI 规范和设计约束。 |
| `requirement-doc-generator` | 生成项目化需求文档，补齐背景、范围、边界、验收和风险。 |
| `setup-dm-mcp` | 为当前项目注册达梦数据库 MCP 服务，并写入项目与 Claude 所需配置。 |
| `setup-redis-mcp` | 为当前项目复用或创建 Redis MCP 服务，并补齐连接配置与授权设置。 |
| `setup-taos-mcp` | 为当前项目复用或创建 TDengine（TaoS）MCP 服务，并补齐连接配置与授权设置。 |

## 使用方式

1. 在会话中直接描述需求，命中技能适用场景时，Claude 会按技能说明执行。
2. 如果要明确触发某个技能，可以直接提到技能目录名或对应任务名称；带显式调用约束的技能，只有在用户明确点名后才可使用。
3. 使用前优先查看目标目录的 `SKILL.md`，确认输入要求、执行流程、输出格式和安全边界。
4. 新增技能时，至少创建 `技能目录/SKILL.md`，写清技能名称、适用场景、步骤、限制和交付物。

## 编写建议

- 技能说明优先写清“触发条件、确认节点、执行顺序、禁止行为”。
- 涉及固定文档、模板或参考资料时，统一放到技能目录并在 `SKILL.md` 中引用。
- Git、数据库、自动化这类高风险操作，要把用户确认点和安全红线写明确。
- 技能应沉淀可复用流程，不要夹带只适用于单次任务的临时上下文。

## 备注

- README 用于快速浏览目录用途与技能清单，具体执行规则仍以各技能目录下的 `SKILL.md` 为准。
- 新增、删除或重命名技能目录后，应同步更新本文件，避免目录说明过期。
