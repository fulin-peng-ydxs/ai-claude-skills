...
| `auto-plan-dev` | 仅在用户显式点名时，把已确认需求文档或功能需求生成为带需求映射、原型承接和页面技术实现约束的开发计划。 |
...
| `requirement-doc-generator` | 基于已确认需求闭环和项目现状，生成带稳定需求编号、页面交互约束和必要 `prototypes/` 原型文件的正式 `requirement.md`。 |
...
- 开发计划类技能除了任务状态，还要维护任务实际落地情况和测试验证的具体完成情况，确保计划更新能反映真实实现、验证结果和遗留缺口。
...
- 需求文档类技能如果输出 HTML 原型，原型中的交互控件必须与需求定义一致；说明性内容要与正式业务界面有明确视觉区分，避免被误实现为线上功能。
